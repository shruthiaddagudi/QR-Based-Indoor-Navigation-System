...
export default function Home() {
  ...
  }, [isReady, prefs.lastFloor, prefs.hasUnlockedVoice, prefs.debugMode, debugMode, switchFloor, toggleDebug]);
  ...
  useEffect(() => {
    if (isLiveTracking && liveTracking.position && liveTracking.floor !== currentFloor.id) {
      ...
      setLiveMessage(
        `Now on ${FLOORS.find((f) => f.id === liveTracking.floor)?.label ?? "the new floor"}.`
      );
    }
  }, [liveTracking.floor, liveTracking.position, isLiveTracking, currentFloor.id, switchFloor, updatePref]);

  // Auto-calibrate floor and position in live tracking when route step floor changes (e.g. climbed steps)
  useEffect(() => {
    if (!isLiveTracking || !route?.found || !currentStep) return;

    if (currentStep.floor !== liveTracking.floor) {
      const node = route.allNodes.find((n) => n.id === currentStep.nodeId);
      if (node && node.floor === currentStep.floor) {
        liveTracking.calibrate(node.position, currentStep.floor);
      }
    }
  }, [currentStep, isLiveTracking, route, liveTracking.floor, liveTracking.calibrate]);
