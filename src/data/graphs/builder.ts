...
export class GraphEngine {
  ...
  private getRoomDoorwayPosition(room: any, junction: { x: number; y: number }): { x: number; y: number } {
    const { x, y, w, h } = room;
    const jx = junction.x;
    const jy = junction.y;

    const px = Math.max(x, Math.min(x + w, jx));
    const py = Math.max(y, Math.min(y + h, jy));
  ...
  private processFloor(floorData: FloorMapData) {
    ...
    floorData.rooms.forEach((r) => {
      ...
      const pos = junc ? this.getRoomDoorwayPosition(r, junc) : { x: r.cx, y: r.cy };
      ...
      if (junc) {
        const dist = distance(pos.x, pos.y, junc.x, junc.y);
